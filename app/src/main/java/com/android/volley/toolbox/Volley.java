...
public class Volley {
	...
	public static RequestQueue newRequestQueue(Context context, HttpStack stack) {
		File cacheDir = new File(context.getCacheDir(), DEFAULT_CACHE_DIR);   //创建一个缓存目录 并对其进行命名 <Volley>

		String userAgent = "volley/0";   //用户代理名
		try {
			String packageName = context.getPackageName();
			PackageInfo info = context.getPackageManager().getPackageInfo(packageName, 0);
			userAgent = packageName + "/" + info.versionCode;    //对用户代理命名采取 包名/版本号 进行命名
		} catch (NameNotFoundException e) {
		}

		if (stack == null) {   //创建网络连接栈
