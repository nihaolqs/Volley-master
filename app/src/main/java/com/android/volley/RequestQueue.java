...
public class RequestQueue {
	...
	public <T> Request<T> add(Request<T> request) {
		...
		request.setSequence(getSequenceNumber());  //序列号
		request.addMarker("add-to-queue");   //标记

		// If the request is uncacheable, skip the cache queue and go straight to the network.
		//如果不允许为缓存队列，则为网络队列
		//默认缓存
		if (!request.shouldCache()) {   //是否允许缓存
			mNetworkQueue.add(request);
			return request;
		}

		// Insert request into stage if there's already a request with the same cache key in flight.
		synchronized (mWaitingRequests) {  //mWaitingRequests等待队列
			String cacheKey = request.getCacheKey();   //获取缓存Key值(网址)
			if (mWaitingRequests.containsKey(cacheKey)) {   //检查是否在等待缓存队列里面
				// There is already a request in flight. Queue up.
				Queue<Request<?>> stagedRequests = mWaitingRequests.get(cacheKey);
				if (stagedRequests == null) {
					stagedRequests = new LinkedList<Request<?>>();  //Map里面包含链表通过链表解决Map同Key值问题
				}
				stagedRequests.add(request);
				mWaitingRequests.put(cacheKey, stagedRequests);  //该key值等待队列长度,stagedRequests.size +1
				if (VolleyLog.DEBUG) {
				...
				// flight.
				mWaitingRequests.put(cacheKey, null);  //标记一个请求
